...
if (window.matchMedia('(min-width: 1024px)').matches) {
    ...
    const storiesObserver = new IntersectionObserver(function (entries) {
        entries.forEach(entry => {
            if (entry.target === document.querySelector('.dailies:first-child')) {
                storiesLeftButton.style.display = (entry.isIntersecting) ? 'none' : 'unset';
            } else if (entry.target === document.querySelector('.dailies:last-child')) {
    ...
    storiesObserver.observe(document.querySelector('.dailies:first-child'));
    storiesObserver.observe(document.querySelector('.dailies:last-child'));
}
