...
aev --version
aev-admin startproject mysite
aev create-app <APP_NAME>
...
const App = () => {
  ...
  const themeVars = theme === 'dark' ? {
    ...
    field: { backgroundColor: '#141b23', color: '#F4F4F4', fontWeight: 'normal' }, //#222333
    cursor: { animation: '1.02s blink-dark step-end infinite' } } :
  {
    /* B, BSOD Mode */
    app: { backgroundColor: '#000' },
    terminal: { boxShadow: '0 2px 5px #FFF' },
    window: { backgroundColor: '#808080', color: '#FFF' },
    field: { backgroundColor: '#0414a7', color: '#FFF', fontWeight: 'bold' },
    cursor: { animation: '1.02s blink-bsod step-end infinite' } };
    /* L, Light Mode
...
const Terminal = ({ theme, setTheme }) => {
  const maximized = React.useState(false); // Full Screen mode 
  const setTitle = React.useState('EROS v2.1');
...
class Field extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      fieldHistory: [{ text: 'Λ L I Ξ V Systems v2.1 [BETA]' }, { text: 'Type HELP to see the full list of commands.', hasBuffer: true }],
      userInput: '',
      isMobile: false };
      ...
      purpose: 'Starts a new instance of the React Terminal.' },
    {
      ...
      'B, BSOD..................Sets the color scheme to BSOD mode.',
      '',
      '-s, -save................Saves the setting to localStorage.'] },

    {
      command: 'exit',
      purpose: 'Quits the React Terminal and returns to \'s portfolio page.' },
    {
      command: 'time',
      purpose: 'Displays the current time.' },
    {
      command: 'emir',
      purpose: 'New Function!' },
    {
      command: 'about',
      isMain: true,
      purpose: 'Displays basic information about .' },
    {
      command: 'experience',
      isMain: true,
      purpose: 'Displays information about \'s experience.' },
    {
      command: 'skills',
      isMain: true,
      purpose: 'Displays information about \'s skills as a developer.' },
    {
      command: 'contact',
      isMain: true,
      purpose: 'Displays contact information for .' },
    {
      command: 'projects',
      isMain: true,
      purpose: 'Displays information about what projects  has done in the past.' },
    {
      command: 'project',
      isMain: true,
    ...
    if (typeof window.orientation !== "undefined" || navigator.userAgent.indexOf('IEMobile') !== -1) {
      this.setState(state => ({
        ...
        fieldHistory: [...state.fieldHistory, { isCommand: true }, {
          text: `Unfortunately due to this application being an 'input-less' environment, mobile is not supported.`,
          isError: true,
          hasBuffer: true }] }));
  ...
  handleInputExecution(cmd, params = [], flags = []) {
    if (cmd === 'help') {
      ...
      () => window.open('https://aliev.io/'));
    } else if (cmd === 'theme') {
      const { setTheme } = this.props;
      const validParams = params.length === 1 && ['d', 'dark', 'l', 'light', 'b', 'bsod'].some(s => s === params[0]);
      const validFlags = flags.length ? flags.length === 1 && (flags[0] === '-s' || flags[0] === '-save') ? true : false : true;

      if (validParams && validFlags) {
        const themeToSet = params[0] === 'd' || params[0] === 'dark' ? 'dark' : 'bsod';
      ...
      return window.location.href = 'https://aliev.io/';
    } else if (cmd === 'emir') {
      return window.location.href = 'emiraliev.com';
    } else if (cmd === 'time') {
      return this.setState(state => ({
      ...
      return this.setState(state => ({
        fieldHistory: [...state.fieldHistory, { text: [
          'Hey there!',
          `My name is . I'm a software developer based around Washington, DC, specializing in the JavaScript ecosystem. I love programming and developing interesting things for both regular folks and developers alike!`,
...
isCommand && /*#__PURE__*/React.createElement("div", { id: "query" }, "user@eros >"), /*#__PURE__*/
...
React.createElement("div", { id: "query" }, "user@eros >"), /*#__PURE__*/
