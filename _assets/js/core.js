//document.title = "ΛΞV | " + (window.location.pathname.split('/')[3]).toUpperCase(); // Set Title

// Set Title
window.addEventListener("focus", () => {
  document.title = "ΛΞV | " + (window.location.pathname.split('/')[2]).toUpperCase(); // Set Title
})
window.addEventListener("blur", () => {

  const titles = ["ΛΞV | Coding is art.", "ΛΞV | Digital Studio."];
...
//Deletes parameter from URL after "?" statement
...
(function () {
  var cases = {
    gpu_core: function() { console.log('GPU core is on!'); },
    _default: function() { console.log('Cookies is not set'); }
  };
  let cookies = document.cookie.split(';')
  cookies.map((cookie) => decodeURIComponent(cookie.split("=")[0])).forEach((element) => {
    cases[ element.split(' ').join('') ] ? cases[ element.split(' ').join('') ]() : cases._default();
  });
}());
