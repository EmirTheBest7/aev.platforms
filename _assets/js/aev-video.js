    ...
    if (data) {
      if (item.data != undefined) {
        thumbnail.setAttribute(
          "style",
          `background: #FFF; border: 2px solid #FFF; background-image: url(${item.data});background-position-x: ${data.backgroundPositionX}px;background-position-y: ${data.backgroundPositionY}px;--x: ${x}px;display: block;`
        );
        // exit
        return;
      }
